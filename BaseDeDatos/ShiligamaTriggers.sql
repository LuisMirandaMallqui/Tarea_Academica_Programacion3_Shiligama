...
BEGIN
    ...
    SET NEW.FECHA_RECIBIDO = COALESCE(NEW.FECHA_RECIBIDO, NEW.FECHA_HORA, NOW());

    IF NEW.ESTADO_PEDIDO = 'EN_PROCESO' THEN
        SET NEW.FECHA_EN_PREPARACION = COALESCE(NEW.FECHA_EN_PREPARACION, NOW());
    END IF;

    IF NEW.ESTADO_PEDIDO = 'LISTO' THEN
        SET NEW.FECHA_EN_PREPARACION = COALESCE(NEW.FECHA_EN_PREPARACION, NOW());
        SET NEW.FECHA_LISTO = COALESCE(NEW.FECHA_LISTO, NOW());
    END IF;

    IF NEW.ESTADO_PEDIDO = 'EN_CAMINO' THEN
        SET NEW.FECHA_EN_PREPARACION = COALESCE(NEW.FECHA_EN_PREPARACION, NOW());
        SET NEW.FECHA_LISTO = COALESCE(NEW.FECHA_LISTO, NOW());
        SET NEW.FECHA_EN_CAMINO = COALESCE(NEW.FECHA_EN_CAMINO, NOW());
    END IF;
    ...
    IF NEW.ESTADO_PEDIDO = 'ENTREGADO' THEN
        ...
        SET NEW.FECHA_LISTO = COALESCE(NEW.FECHA_LISTO, NOW());
        SET NEW.FECHA_EN_CAMINO = COALESCE(NEW.FECHA_EN_CAMINO, NOW());
        SET NEW.FECHA_ENTREGADO = COALESCE(NEW.FECHA_ENTREGADO, NOW());
    END IF;

    IF NEW.ESTADO_PEDIDO = 'RECOGIDO' THEN
        SET NEW.FECHA_EN_PREPARACION = COALESCE(NEW.FECHA_EN_PREPARACION, NOW());
        SET NEW.FECHA_LISTO = COALESCE(NEW.FECHA_LISTO, NOW());
        SET NEW.FECHA_RECOGIDO = COALESCE(NEW.FECHA_RECOGIDO, NOW());
    END IF;

    IF NEW.ESTADO_PEDIDO IN ('RECHAZADO', 'CANCELADO') THEN
        SET NEW.FECHA_CANCELADO = COALESCE(NEW.FECHA_CANCELADO, NOW());
    END IF;
END$$
...
BEGIN
    ...
    IF NEW.ESTADO_PEDIDO <> OLD.ESTADO_PEDIDO THEN
        ...
        IF NEW.ESTADO_PEDIDO = 'RECIBIDO' THEN
            SET NEW.FECHA_RECIBIDO = COALESCE(NEW.FECHA_RECIBIDO, NOW());
        END IF;
