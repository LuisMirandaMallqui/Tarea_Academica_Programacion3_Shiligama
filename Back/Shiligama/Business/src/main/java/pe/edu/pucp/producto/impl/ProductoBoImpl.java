...
public class ProductoBoImpl implements ProductoBo {
    ...
    public int eliminar(int id) throws Exception {
        if (id <= 0) throw new Exception("El ID del producto debe ser mayor que cero.");
    ...
    public List<Producto> buscarPaginado(Integer categoriaId, String q,
        ...
        if (pagina < 1) throw new Exception("La pagina debe ser mayor o igual a 1.");
        if (tamano < 1 || tamano > 100) throw new Exception("El tamano debe estar entre 1 y 100.");
        if (precioMin != null && precioMin < 0) throw new Exception("El precio minimo no puede ser negativo.");
        if (precioMax != null && precioMax < 0) throw new Exception("El precio maximo no puede ser negativo.");
        if (precioMin != null && precioMax != null && precioMin > precioMax)
            throw new Exception("El precio minimo no puede ser mayor que el maximo.");
        return daoProducto.buscarPaginado(categoriaId, q, precioMin, precioMax, soloPromo, pagina, tamano);
    }
